# preprocess.py - Ajouter le sous-échantillonnage chrominance
import cv2
import numpy as np
...
def subsample_chrominance(ycbcr_img, ratio=2):
    """
    Sous-échantillonnage 4:2:0 (réduction de moitié des canaux Cb et Cr)
    """
    Y, Cb, Cr = cv2.split(ycbcr_img)
    
    # Sous-échantillonnage
    Cb_subsampled = cv2.resize(Cb, (Cb.shape[1] // ratio, Cb.shape[0] // ratio), 
                               interpolation=cv2.INTER_LINEAR)
    Cr_subsampled = cv2.resize(Cr, (Cr.shape[1] // ratio, Cr.shape[0] // ratio),
                               interpolation=cv2.INTER_LINEAR)
    
    return Y, Cb_subsampled, Cr_subsampled

def upsample_chrominance(Cb_small, Cr_small, target_shape):
    """Ré-échantillonnage pour reconstruction"""
